...
- Prevented race condition in remote retry timer that could complete a Future twice.
- Added guards to avoid overlapping remote loads and handle remote load errors safely.
- Documentation corrections (init snippet, package name, example run command).

### Added

- Regression test covering slow remote loads using fake timers.
